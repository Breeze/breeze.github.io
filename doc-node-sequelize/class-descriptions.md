...
- **importMetadata**(*breezeMetadata*):  Instance method that imports breeze metadata into the manager. This metadata is needed for many of the other operations that the manager performs.
	- *parameter*: breezeMetadata: This can be any metadata that conforms to the breeze metadata specification and may be in either string or json format. For more information, see [Breeze Metadata](/doc-js/metadata)

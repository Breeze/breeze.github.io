...
- **BeforeSaveEntities**
	...
	`BeforeSaveEntities` interception point for business logic to be applied after the entites have been materialized from the HTTP request, but **before** they are saved to the database. This is a good place for [server-side validation](http://www.breezejs.com/documentation/server-side-validation).
...
## Getting Started
The following instructions assume you're using Visual Studio 2012. Please look at the [NorthBreeze sample application](http://www.breezejs.com/samples/northbreeze), where the following steps have already been performed:
...
1. Write your [WebAPI controller](http://www.breezejs.com/documentation/web-api-controller) class.  It uses your derived NHContext class (described above) to implement the SaveChanges and Metadata methods, as well as a method that exposes each of your entities for querying. The [BreezeNHController] attribute adds a filter to each of the query methods, applyingOData parameters to the query before execution.
...
1. Write your Breeze client. Dive into the [Breeze documentation](http://www.breezejs.com/documentation/introduction) for a full description.  
...
We intend to fix this in the future by auto-generating the foreign key properties on the client (using a custom [JsonResultsAdapter](http://www.breezejs.com/documentation/web-service-data-mapping)), and adding them to the server-generated metadata.
